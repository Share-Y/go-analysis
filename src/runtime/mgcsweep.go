...
// deductSweepCredit 扣除分配的span字节大小的扫描信用
// 必须在span分配前执行, 确保系统有足够的信用, 如果需要, 它会进行扫描防止负债
// 如果调用方本身也将扫描页面(比如一个大的对象), 它可以通过一个 非零callerSweepPages 保留这些未扫描页面
// deductSweepCredit 是扫描信用体系的核心, 在GC循环之间的扫描阶段, 它使用垃圾收集器收集的统计信息来完成足够的扫描量, 以便在并发过程中扫描所有页面
