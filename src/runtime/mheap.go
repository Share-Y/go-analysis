...
const (
	...
	mSpanManual            // allocated for manual management (e.g., stack allocator)	// 手动管理
)
...
type mspan struct {
	...
	// helps performance.
	nelems uintptr // number of object in the span. // span中允许存放的对象数
	...
	allocCount  uint16        // number of allocated objects // 已分配对象数量
...
// reclaim 扫描和回收至少 npage 个页到堆中
// 在分配 npage 个页之前调用它来控制增长
// reclaim 实现了页回收器的一半
// h.lock必须处于释放状态
...
const (
	spanAllocHeap          spanAllocType = iota // heap span // 堆 span
...
// alloc 从GC的堆中分配一个新的npage页数的span
// spanclass 表示跨度大小等级和可扫描性
// needzero = true 返回的span内存大小为0
func (h *mheap) alloc(npages uintptr, spanclass spanClass, needzero bool) *mspan {
	// Don't do any operations that lock the heap on the G stack.
	// It might trigger stack growth, and the stack growth code needs
	// to be able to allocate heap.
	// 不要执行任何锁定G栈上的堆的操作, 否则可能会触发栈增长, 栈增长代码需要能分配堆
	var s *mspan
	systemstack(func() {
		// To prevent excessive heap growth, before allocating n pages
		// we need to sweep and reclaim at least n pages.
		// 为了防止堆过度增长, 需要在分配n个页之前, 我们需要扫描和回收至少n个页
		if h.sweepdone == 0 {
			h.reclaim(npages)
		}
		// 分配span
...
//
//go:systemstack
// allocSpan 分配一个拥有 npages 内存的 mspan
// typ.manual() == false 分配堆span并更新堆记录, manual == true, 分配一个手动管理的span(不考虑spanClass)
// 并且调用方负责与其使用span相关的任何记录
// 无论哪种方式 allocSpan 都会将新分配的span的 mSpanStateBox 更新
func (h *mheap) allocSpan(npages uintptr, typ spanAllocType, spanclass spanClass) (s *mspan) {
	...
	// 如果分配足够小, 请尝试页面缓存, 页面缓存不支持对齐分配, 因此如果我们需要提供一个物理页面对齐的栈分配就不能使用这个
	pp := gp.m.p.ptr()
	if !needPhysPageAlign && pp != nil && npages < pageCachePages/4 { // npages < 64
		c := &pp.pcache
...
HaveSpan:
	...
	if typ.manual() {
		...
		// 在任何地方发布span之前, 必须设置span属性 因为我们没有持有堆锁
