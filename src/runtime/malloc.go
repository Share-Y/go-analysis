...
func (c *mcache) nextFree(spc spanClass) (v gclinkptr, s *mspan, shouldhelpgc bool) {
	...
	if freeIndex == s.nelems {
		...
		// 向mcentral中申请新的span, 因为是新申请的 所以要打工还债了
		c.refill(spc)
		shouldhelpgc = true
		s = c.alloc[spc]
...
func mallocgc(size uintptr, typ *_type, needzero bool) unsafe.Pointer {
	...
	// assistG 是为这个分配收费的G, 如果GC当前未处于活动状态, 则 assistG = nil
	var assistG *g
	...
	// 如果 *_type = nil 或者 *_type.ptrdata == 0 这就是一个无指针的对象
	noscan := typ == nil || typ.ptrdata == 0
	if size <= maxSmallSize { // <=32kb
		if noscan && size < maxTinySize { // 无指针 && 小于16b
			// 微对象分配
			//
			...
			//
			// 微型分配器组合了几个微型分配请求存储到单个内存块中。生成的内存块将在所有子对象都无法访问时才释放
			// 子对象 必须是noscan(不含指针)，这确保潜在的内存浪费量是有限的。
			//
			...
			//
			// 用于组合(maxTinySize)的内存块大小是可调的. 默认设置为16字节, 最坏情况下内存损耗2倍(当除一个子对象以外的所有子对象都无法访问时)
			// 8字节将不会导致任何浪费, 但提供的合并机会更少. 32字节提供了更多的组合机会, 但在最坏的情况下, 可能导致4倍的内存浪费
			// 无论内存块大小如何设置, 最佳方案都是8的倍数
			//
			// Objects obtained from tiny allocator must not be freed explicitly.
			// So when an object will be freed explicitly, we ensure that
			// its size >= maxTinySize.
			//
			// 不能显式释放从微小分配器获取的对象. 因此, 当一个对象被显式释放时, 我们可以确定其大小>=maxTinySize。
			//
			...
			//
			// 微型分配器的主要目标是小字符串和 逃逸变量。数据显示分配器将分配数量减少约12%，并且将堆大小减少约20%。
			off := c.tinyoffset
